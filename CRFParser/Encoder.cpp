...
double LencodeMarginals(const int length,const double* scores,const int ln, double* marginals, double* marginals_pure)
{
	...
	for(int i=0;i<length;i++){
		for(int j=i+1;j<length;j++){
			...
			int key_assign_pure = get_index2(length, i, j, 0, 1);
			marginals_pure[key_assign_pure] = exp(tmp_mscore);
			...
			key_assign_pure = get_index2(length, j, i, 0, 1);
			marginals_pure[key_assign_pure] = exp(tmp_mscore);
			for(int zl=0;zl<ln;zl++){
