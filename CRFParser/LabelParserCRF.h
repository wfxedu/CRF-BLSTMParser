...
struct ParserBuilderLabel {
	...
	Parameters* p_Bo[2];
	Parameters* p_WoL[2];
	Parameters* p_WoBiasL[2];
	Parameters* p_BoL[2];
	Parameters* p_segment_gard[2];
	feature feaarray[10];

	double* temp_scores;
	vector<Expression*> exp_score_nodes;
	vector<Expression*> exp_score_labels;

	LookupParameters* p_dist; // relation embeddings
	LookupParameters* p_r; // relation embeddings
	enum { 
		FEA_MIN, FEA_MAX, FEA_DIS, FEA_S0, FEA_S1, FEA_S2,
		FEA_HBIAS, FEA_HS, FEA_MBIAS, FEA_MS
	};

	explicit ParserBuilderLabel(parser_config& cfg);
