...
vector<int>* decodeProjective(int length, double* scores)
{
	...
	int u = -2;
	for (int k = 1; k < length; k++) {
		double tmp = scores_table[get_index(length, 1, k, E_LEFT, E_COM)]
			+ scores_table[get_index(length, k, length - 1, E_RIGHT, E_COM)] +
			scores[get_index2(length, 0, k)];
		if (tmp >= max_score) {
			max_score = tmp;
			u = k;
		}
	...
	int the_ind = get_index(length, 0, length - 1, E_RIGHT, E_COM);
	vector<int>* result = new vector<int>(length, -1);
	(*result)[u] = 0;
	(*result)[0] = -1;
	fill_result(length, which, result, 1, u, E_LEFT, E_COM);
	fill_result(length, which, result, u, length - 1, E_RIGHT, E_COM);
	delete[]scores_table;
	delete[]which;
	return result;
}
